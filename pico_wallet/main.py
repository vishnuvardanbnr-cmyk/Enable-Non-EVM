    ...
    def save_chains(self, chains):
        ...
        try:
            with open(CHAINS_FILE, "w") as f:
                json.dump(chains, f)
            return {"success": True, "message": "Chain preferences saved"}
        except Exception as e:
            return {"error": f"Failed to save chains: {str(e)}"}
...
def main():
    ...
    while True:
        try:
            ...
            elif action == "save_chains":
                chains = cmd.get("chains", [])
                result = wallet.save_chains(chains)
            elif action == "get_chains":
                result = wallet.get_chains()
            elif action == "ping":
                result = {"pong": True}
            else:
